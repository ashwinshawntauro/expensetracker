...
function App() {
    ...
    const fetchWallet = () => {
        return fetch("http://localhost:4000/api/wallets")
        .then((res) => res.json())
        .then((d) => setData2(d))
    }
    useEffect(() => {
                    ...
                    <div>
                        ...
                        <div className="wallet-grid my-3">
                            <div className="bg-backdrop text-primary shadow rounded p-2">
                                ...
                                {data2.map((dataObj, index) => {return (<small className="fs-4 fw-semibold">{dataObj.ele[0].ch}</small> )})}
                            </div>

                            <div className="bg-backdrop text-primary shadow rounded p-2">
                                <div className="fw-bold">Bank</div>
                                {data2.map((dataObj, index) => {return (<small className="fs-4 fw-semibold">{dataObj.ele[0].bk}</small> )})}  
                            </div>
                        </div>
                        ...
                        return (
                            <div className="d-flex flex-column my-3 text-center" key={index}>
                                <div className="d-flex flex-row justify-content-between trans py-2">
                                    <img className="border border-2 rounded bg-white" src={`${dataObj.logo}`}></img>
                                    <div>
                                        <div className="fw-semibold">{dataObj.category}</div>
                                        <small>{dataObj.date}</small>
                                    </div>
                                    <div className={`fw-semibold ${dataObj.value>0 ? "text-success" :"text-danger"}`}>{dataObj.value}</div>
                                </div>
